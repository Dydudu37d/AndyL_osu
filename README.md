...
项目包含了训练图像数据，您可以使用这些数据重新训练模型或扩展模型功能。

1. 准备训练数据，放入`train_img`目录下的对应子目录
2. 修改并运行`analyze_data.py`脚本进行数据验证
3. 根据需要调整模型结构（在`main.py`中的`OsuNet`类）
4. 重新训练模型并保存到`models`目录（训练完之后会自动保存）

## 注意事项

1. 程序运行时，请确保OSU游戏窗口处于激活状态
2. 建议在窗口模式下运行OSU游戏，以获得最佳检测效果
